...
function buildEmailHtml(body: SubmissionBody): string {
  ...
  if (body.packageLabel) {
    ...
    selectionHtml = `<p style="color:#666;font-style:italic">No package selected</p>`
  }

  const messageHtml = body.message
    ? `<div style="margin-top:20px;padding:16px;background:#f9f9f9;border-radius:8px"><p style="margin:0 0 4px;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;color:#999">Message</p><p style="margin:0;color:#333">${body.message}</p></div>`
    : ""
    ...
    <div style="background:#fff;padding:24px;border-radius:0 0 12px 12px;border:1px solid #e5e5e5;border-top:none">
      <table style="width:100%;margin-bottom:20px">
        ...
        <tr>
          ...
          <td style="padding:4px 0;font-size:14px;color:#111;text-align:right"><a href="mailto:${body.clientEmail}" style="color:#111">${body.clientEmail}</a></td>
        </tr>
      </table>
